...
def shortest_path(source, targets, occupied):
    result = []
    best = None
...
def choose_target(position, targets, occupied):
    ...
    if position in targets:
        return position
    paths = shortest_path(position, targets, occupied)
...
def choose_move(position, target, occupied):
    if position == target:
        return position
    paths = shortest_path(position, {target}, occupied)
    starts = [x[1] for x in paths]
    return min(starts) if starts else None

def adjacent(sources):
...
if __name__ == '__main__':
    ...
    #rounds, hp = World(lines).run()
    #print(rounds * hp)

    for elf_attack in count(4):
        try:
            print(elf_attack)
            rounds, hp = World(lines, elf_attack).run()
            print(rounds*hp)
            break
        except Exception:
            pass
